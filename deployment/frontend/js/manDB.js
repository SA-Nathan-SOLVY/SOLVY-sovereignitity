...
class MANDatabase {
  ...
  async init() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        this._createStores(db, upgradeTransaction);
      };
    });
  }
   ...
   */
  _createStores(db, upgradeTransaction) {
    ...
    if (!db.objectStoreNames.contains('transactions')) {
      ...
      txStore = upgradeTransaction.objectStore('transactions');
    }
    // Ensure all indexes exist (safe for upgrades)
    if (!txStore.indexNames.contains('timestamp')) txStore.createIndex('timestamp', 'timestamp', { unique: false });
    if (!txStore.indexNames.contains('amount')) txStore.createIndex('amount', 'amount', { unique: false });
    if (!txStore.indexNames.contains('merchant')) txStore.createIndex('merchant', 'merchant', { unique: false });
    if (!txStore.indexNames.contains('status')) txStore.createIndex('status', { unique: false });
    if (!txStore.indexNames.contains('syncStatus')) txStore.createIndex('syncStatus', 'syncStatus', { unique: false });

    // Store 2: Categories (pre-defined spending categories)
    if (!db.objectStoreNames.contains('categories')) {
      const catStore = db.createObjectStore('categories', { keyPath: 'name' });
      console.log('[MANDB] Created categories store');
    }

    // Store 3: Aggregated Metrics (local cache of pool balances)
    ...
    // Store 4: Proposals (active votes)
    ...
    // Store 5: Member Votes (anonymous voting record)
    ...
    // Store 7: Audit Log (immutable local record)
   ...
   * @param {string} transaction.id - UUID (optional, auto-generated if omitted)
   * @param {string} transaction.timestamp - ISO string (optional, defaults to now)
   * @param {number} transaction.amount - Transaction amount
   * @param {string} transaction.merchant - Merchant name
   * @param {string} transaction.category - Spending category (optional, defaults to "Uncategorized")
   * @param {string} transaction.status - "pending" or "settled" (optional, defaults to "pending")
   * @param {string} transaction.syncStatus - "synced" or "pending_sync" (optional, defaults to "pending_sync")
  ...
  async addTransaction(transaction) {
    const tx = {
      ...
      syncStatus: transaction.syncStatus || 'pending_sync',
   ...
   * @param {string} filters.merchant - Partial merchant name match
   * @param {string} filters.status - "pending" or "settled"
   ...
   */
  async updateTransactionCategory(id, newCategory) {
    const tx = await this._get('transactions', id);
    if (!tx) {
      console.error('[MANDB] Transaction not found:', id);
      return false;
    }
    ...
    await this._addAuditEntry('transaction_category_update', 0, id);
    return true;
  }
  ...
  async deleteTransaction(id) {
    const tx = await this._get('transactions', id);
    if (!tx) {
      console.error('[MANDB] Transaction not found for deletion:', id);
      return false;
    }

    await this._delete('transactions', id);
    await this._addAuditEntry('transaction_deleted', tx.amount, id);
    return true;
  }
  ...
  async getPendingSyncTransactions() {
    const allTransactions = await this._getAll('transactions');
    return allTransactions.filter(tx => tx.syncStatus === 'pending_sync');
  }
  ...
  async markAsSynced(id) {
    const tx = await this._get('transactions', id);
    if (!tx) {
      console.error('[MANDB] Transaction not found for sync:', id);
      return false;
    }
    ...
    await this._put('transactions', tx);
    await this._addAuditEntry('transaction_marked_synced', 0, id);
    return true;
  }
  ...
  /**
   * Seed default spending categories if the store is empty
   * @returns {Promise<void>}
   */
  async seedDefaultCategories() {
    ...
    const existing = await this._getAll('categories');
    if (existing.length === 0) {
      for (const cat of defaults) {
        try {
          await this._add('categories', cat);
        } catch (e) {
          // May already exist, ignore
        }
      }
      console.log('[MANDB] Seeded default categories');
    }
  }
}
...
/**
 * Standalone initDB function — opens database and seeds defaults.
 * Use this when you need to ensure the DB is ready before operations.
 * @returns {Promise<MANDatabase>}
 */
async function initDB() {
  if (manDB.isReady) return manDB;
  await manDB.init();
  await manDB.seedDefaultCategories();
  return manDB;
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { MANDatabase, manDB, initDB };
}
