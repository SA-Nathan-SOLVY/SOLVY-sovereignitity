...
export default function createCardRouter(pool?: Pool) {
  const router = Router();
  ...
  // ── Webhook (legacy path — prefer /webhooks/lithic) ────────────────────────
  ...
  router.post('/webhook', async (req: Request, res: Response) => {
    try {
      ...
      console.log('[Lithic Webhook /api/card/webhook]', event.type, event.data?.token || event.data?.card_token || '');

      if (pool) {
        await handleLithicWebhookEvent(pool, event).catch((err: any) => {
