  ...
  return (
    <nav className="unified-navbar">
      <div className="unified-nav-container">
        {/* LEFT MAJOR DROPDOWN — logo opens the full ecosystem menu */}
        <div
          className={`nav-logo-dropdown nav-dropdown ${ecosystemOpen ? 'open' : ''}`}
          onMouseEnter={() => setEcosystemOpen(true)}
          onMouseLeave={() => setEcosystemOpen(false)}
        >
          <div className="unified-nav-logo-wrap">
            ...
            <button
              ...
              aria-label="Open SOLVY ecosystem menu"
              aria-expanded={ecosystemOpen}
              aria-controls="ecosystem-mega-menu"
              onClick={() => setEcosystemOpen(!ecosystemOpen)}
            >
              ▼
            </button>
          </div>
          <div className="dropdown-menu mega-menu" id="ecosystem-mega-menu">
            <div className="mega-menu-header">SOLVY Ecosystem™</div>

            <div className="mega-menu-section-label">💳 SOLVY CARD</div>
            <Link to="/" onClick={() => setEcosystemOpen(false)}>SOLVY Home</Link>
            <Link to="/apply" onClick={() => setEcosystemOpen(false)}>🆕 New? Start Here</Link>
            <Link to="/apply" onClick={() => setEcosystemOpen(false)}>Resource Hub</Link>
            <Link to="/apply" onClick={() => setEcosystemOpen(false)}>Apply for Card</Link>
            <Link to="/banking" onClick={() => setEcosystemOpen(false)}>Banking Portal</Link>
            <Link to="/card-android-app" onClick={() => setEcosystemOpen(false)}>Android App Info</Link>
            <Link to="/card-ios-app" onClick={() => setEcosystemOpen(false)}>iOS App Info</Link>
            <Link to="/prelaunch" onClick={() => setEcosystemOpen(false)}>Prelaunch Commit</Link>

            <div className="mega-menu-divider" />
            <div className="mega-menu-section-label">🏪 PILOT BUSINESSES</div>
            <Link to="/ebl" onClick={() => setEcosystemOpen(false)}>Evergreen Beauty Lounge</Link>
            <Link to="/sps" onClick={() => setEcosystemOpen(false)}>SPS Joint Venture</Link>

            <div className="mega-menu-divider" />
            <div className="mega-menu-section-label">📚 EDUCATION & MISSION</div>
            <Link to="/manifesto" onClick={() => setEcosystemOpen(false)}>About · Manifesto</Link>
            <Link to="/heritage" onClick={() => setEcosystemOpen(false)}>Heritage</Link>
            <Link to="/sovereignitity" onClick={() => setEcosystemOpen(false)}>SOVEREIGNITITY™</Link>
            <Link to="/decidey" onClick={() => setEcosystemOpen(false)}>DECIDEY</Link>
            <Link to="/data-marketplace" onClick={() => setEcosystemOpen(false)}>Data Marketplace</Link>
            <Link to="/moli" onClick={() => setEcosystemOpen(false)}>MOLI Strategy</Link>

            <div className="mega-menu-divider" />
            <div className="mega-menu-section-label">📊 MAN · OPERATIONS</div>
            <Link to="/man" onClick={() => setEcosystemOpen(false)}>MAN Dashboard</Link>
            <Link to="/man" onClick={() => setEcosystemOpen(false)}>MAN Simple — Founding Members</Link>
            <Link to="/comms" onClick={() => setEcosystemOpen(false)}>Comms Center</Link>
            <Link to="/mailbox" onClick={() => setEcosystemOpen(false)}>Mailbox</Link>
            <Link to="/presentations" onClick={() => setEcosystemOpen(false)}>Presentations</Link>
          </div>
        </div>
        
        {/* SOLVY ECOSYSTEM NAVBAR — scoped quick-access dropdowns only.
            The full ecosystem lives in the two MAJOR dropdowns (logo + hamburger);
            inline items here stay scoped/related so they don't duplicate that menu.
            CENTER (Public / Education):          About · DECIDEY → SOVEREIGNITITY
            RIGHT  (Members-only / Transparency): MAN
            Pilots (EBL · SPS) intentionally not repeated here — they're in the mega-menu.
          ...
          <div className="nav-group-center">
            ...
            >
              ...
              <div className="dropdown-menu">
                ...
                <Link to="/data-marketplace" onClick={() => setAboutDropdownOpen(false)}>🔐 Data Marketplace</Link>
                <Link to="/my-data" onClick={() => setAboutDropdownOpen(false)}>📁 My Data</Link>
              </div>
            </div>
        ...
        {/* RIGHT MAJOR DROPDOWN — hamburger opens the full ecosystem menu */}
        <button
      ...
      <div className={`mobile-menu ${mobileMenuOpen ? 'open' : ''}`}>
        ...
        <Link to="/card-android-app" onClick={closeMobileMenu}>🤖 Android App Info</Link>
        <Link to="/card-ios-app" onClick={closeMobileMenu}>📱 iOS App Info</Link>
        ...
        <Link to="/data-marketplace" onClick={closeMobileMenu}>🔐 Data Marketplace</Link>
        <Link to="/my-data" onClick={closeMobileMenu}>📁 My Data</Link>
