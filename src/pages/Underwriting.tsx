...
interface EblTransaction {
  ...
  payment_type: string
  interchange_cooperative: number
  interchange_ebl: number
  interchange_platform: number
  created_at: string
}
...
export default function Underwriting() {
  ...
  const [eblTransactions, setEblTransactions] = useState<EblTransaction[]>([])
  ...
  useEffect(() => {
    ...
    setEblLoading(true)
    fetch('/api/ebl/transactions', { headers: { 'x-staff-token': staffToken } })
      .then((r) => r.json())
      .then((data) => setEblTransactions(data.transactions ?? []))
      .catch(() => setEblTransactions([]))
      .finally(() => setEblLoading(false))
  }, [verified, staffToken])
  ...
  function exportEblCSV() {
    if (!eblTransactions.length) return
    const headers = ['Date', 'Customer', 'Service', 'Amount', 'Method', 'Coop Split', 'EBL Split', 'Platform Split']
    const rows = eblTransactions.map((t) => [
      t.created_at?.slice(0, 10),
      t.customer_name || '',
      t.service_type || '',
      (Number(t.amount_total) / 100).toFixed(2),
      t.payment_type === 'solvy' ? 'SOLVY' : (t.payment_type || 'Other'),
      (Number(t.interchange_cooperative) / 100).toFixed(4),
      (Number(t.interchange_ebl) / 100).toFixed(4),
      (Number(t.interchange_platform) / 100).toFixed(4),
    ])
    const csv = [headers, ...rows].map((r) => r.map(csvEscape).join(',')).join('\n')
    const a = document.createElement('a')
    a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }))
    a.download = `ebl_transactions_${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
  }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.container}>
        ...
        <section style={s.section}>
          ...
          <div style={s.coopGrid}>
            <div style={{ ...s.coopCard, background: 'linear-gradient(135deg, #1e3a5f, #0f1e2c)' }}>
              ...
              <div style={s.coopLabel}>Membership Pool</div>
              <div style={s.coopDesc}>Held for members — distribution decided later, at maturity, with DECIDEY financial education</div>
            </div>
            ...
            <div style={{ ...s.coopCard, background: 'linear-gradient(135deg, #064e3b, #022c22)' }}>
              ...
              <div style={s.coopLabel}>Sovereign Wealth Fund</div>
        ...
        {/* Card Issuance Account Structure */}
        <section style={s.section}>
        ...
        <section style={s.section}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #0f1e2c 0%, #1e3a5f 100%)', borderRadius: '16px', padding: '28px 32px', color: 'white', marginBottom: '24px' }}>
            <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#ffb347', marginBottom: '12px' }}>
              "Our infrastructure partner provides the engine. We provide the intelligence."
            </div>
...
└──────────────────────────────────────────────────────────────┘`}
        ...
        {/* EBL Transactions */}
        <section style={s.section}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginTop: 0, marginBottom: '24px', paddingBottom: '12px', borderBottom: '1px solid #f0f4f8' }}>
            <h2 style={{ ...s.sectionTitle, margin: 0, padding: 0, border: 'none' }}>💳 EBL Payment History — Pilot #1 Merchant Volume</h2>
            {eblTransactions.length > 0 && (
              <button onClick={exportEblCSV} style={s.exportBtn}>⬇️ Export CSV</button>
            )}
          </div>
          <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '12px', padding: '14px 20px', marginBottom: '20px', fontSize: '0.85rem', color: '#166534', lineHeight: 1.6 }}>
            <strong>Live merchant activity at Evergreen Beauty Lounge.</strong> Every completed service payment is recorded here with the full 3-way interchange split: 70% Cooperative · 20% EBL · 10% Platform.
          </div>

          {eblLoading ? (
            <div style={{ textAlign: 'center', padding: '32px', color: '#94a3b8' }}>Loading transactions…</div>
          ) : eblTransactions.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '32px', color: '#94a3b8', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
              No transactions recorded yet. Payments processed at EBL will appear here automatically.
            </div>
          ) : (
            <>
              {/* Summary bar */}
              {(() => {
                const total = eblTransactions.reduce((sum, t) => sum + Number(t.amount_total), 0)
                const coop = eblTransactions.reduce((sum, t) => sum + Number(t.interchange_cooperative), 0)
                const ebl = eblTransactions.reduce((sum, t) => sum + Number(t.interchange_ebl), 0)
                const platform = eblTransactions.reduce((sum, t) => sum + Number(t.interchange_platform), 0)
                const solvy = eblTransactions.filter((t) => t.payment_type === 'solvy').length
                return (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '20px' }}>
                    {[
                      { label: 'Total Volume', value: fmt(total / 100), color: '#0f1e2c' },
                      { label: 'Transactions', value: String(eblTransactions.length), color: '#0f1e2c' },
                      { label: 'Cooperative (70%)', value: fmt(coop / 100), color: '#7c3aed' },
                      { label: 'EBL Share (20%)', value: fmt(ebl / 100), color: '#0891b2' },
                      { label: 'Platform (10%)', value: fmt(platform / 100), color: '#16a34a' },
                      { label: 'SOLVY Payments', value: String(solvy), color: '#ffb347' },
                    ].map((card) => (
                      <div key={card.label} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '14px 16px' }}>
                        <div style={{ fontSize: '0.72rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>{card.label}</div>
                ...
                <table style={s.table}>
                  <thead>
                    <tr style={{ background: '#f8fafc' }}>
                      ...
                      <th style={s.th}>Coop (70%)</th>
                      <th style={s.th}>EBL (20%)</th>
                      <th style={s.th}>Platform (10%)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {eblTransactions.map((t, i) => {
                      const isSolvy = t.payment_type === 'solvy'
                      return (
                        <tr key={t.id} style={{ background: i % 2 === 0 ? 'white' : '#fafbfc' }}>
                          <td style={s.td}>{new Date(t.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                          <td style={s.td}>{t.customer_name || '—'}</td>
                          <td style={s.td}>{t.service_type || '—'}</td>
                          <td style={{ ...s.td, fontWeight: 700 }}>{(Number(t.amount_total) / 100).toLocaleString('en-US', { style: 'currency', currency: t.currency?.toUpperCase() || 'USD' })}</td>
                          <td style={s.td}>
                            <span style={{ background: isSolvy ? '#fef3c7' : '#ede9fe', color: isSolvy ? '#92400e' : '#5b21b6', padding: '3px 10px', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 700 }}>
                              {isSolvy ? 'SOLVY' : t.payment_type || 'Other'}
                            </span>
                          </td>
                          <td style={{ ...s.td, color: '#7c3aed', fontWeight: 600 }}>{(Number(t.interchange_cooperative) / 100).toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 4 })}</td>
                          <td style={{ ...s.td, color: '#0891b2', fontWeight: 600 }}>{(Number(t.interchange_ebl) / 100).toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 4 })}</td>
                          <td style={{ ...s.td, color: '#16a34a', fontWeight: 600 }}>{(Number(t.interchange_platform) / 100).toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 4 })}</td>
                        </tr>
                      )
                    })}
                  </tbody>
              ...
              <div style={{ marginTop: '12px', fontSize: '0.75rem', color: '#94a3b8' }}>
                Showing most recent {eblTransactions.length} transaction{eblTransactions.length !== 1 ? 's' : ''}. Interchange calculated at 0.8% of transaction value.
              </div>
            </>
          )}
        </section>
