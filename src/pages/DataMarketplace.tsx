...
import { listTransactions, buildAggregate, getContributorId } from '../lib/localData'
...
export default function DataMarketplace() {
  ...
  const emailSubmitted = !!verifiedEmail
  ...
  const loadMyOptins = async () => {
    try {
      const r = await fetch('/api/data-pools/my-optins', { credentials: 'include' })
  ...
  // Triggered by the verification widget. Load opt-ins on verify, clear on sign-out.
  ...
  const toggleOptin = async (poolId: string) => {
    ...
    try {
      ...
      if (d.success !== false) {
        ...
        const poolName = pools.find((p) => p.id === poolId)?.name
        if (isIn) {
          // Leaving the pool — withdraw any anonymized aggregate this device sent.
          await fetch('/api/data-pools/contribute', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ poolId, contributorId: getContributorId() }),
          }).catch(() => {})
          setMessage({ text: `Left "${poolName}" pool. Your aggregate was withdrawn.`, type: 'success' })
        } else {
          // Joining — contribute a PII-free aggregate computed from local-only data.
          const txs = await listTransactions()
          if (txs.length > 0) {
            const res = await fetch('/api/data-pools/contribute', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ poolId, contributorId: getContributorId(), aggregate: buildAggregate(txs) }),
            }).then((x) => x.json()).catch(() => null)
            setMessage(
              res?.success
                ? { text: `Joined "${poolName}" pool. Your anonymized aggregate was contributed — no raw transactions left your device.`, type: 'success' }
                : { text: `Joined "${poolName}" pool, but your aggregate was not contributed${res?.error ? `: ${res.error}` : '.'} Manage it in My Data.`, type: 'info' }
            )
          } else {
            setMessage({ text: `Joined "${poolName}" pool. Add local transactions in My Data to contribute an aggregate.`, type: 'info' })
          }
        }
      } else {
    ...
    <div style={s.page}>
      ...
      <section style={{ ...s.section, background: 'rgba(147,51,234,0.04)' }}>
        <div style={s.container}>
          ...
          <div style={s.localBanner}>
            <span>📁 Your transactions stay on your device. Joining a pool sends only an anonymized aggregate — manage and review it in <a href="/my-data" style={s.link}>My Data</a>.</span>
          </div>

          <div style={s.emailGate}>
            <MemberVerify
              onChange={handleVerifiedChange}
              prompt="Verify your member email to view and manage your pool opt-ins. Opting in is tied to your confirmed identity."
            />
          </div>
          ...
          {!emailSubmitted && (
            <p style={{ ...s.bodyText, textAlign: 'center' as const, marginTop: '1rem', fontSize: '0.85rem' }}>
              You can browse pools without signing in. Verify your email above to opt in or out.
            </p>
          )}
        </div>
      </section>
...
const s: Record<string, React.CSSProperties> = {
  ...
  localBanner: { background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '12px', padding: '0.9rem 1.25rem', color: '#94a3b8', fontSize: '0.9rem', lineHeight: 1.55, marginBottom: '2rem', textAlign: 'center' as const },
