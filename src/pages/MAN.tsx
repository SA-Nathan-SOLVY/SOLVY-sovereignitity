...
function MAN() {
  ...
  const [expandedVotes, setExpandedVotes] = useState<number | null>(null)
  const [voteHistory, setVoteHistory] = useState<{ voter: string; choice: string; created_at: string }[]>([])

  interface DataPool { id: string; name: string }
  const [dataPools, setDataPools] = useState<DataPool[]>([])
  const [showProposalForm, setShowProposalForm] = useState(false)
  const [propTitle, setPropTitle] = useState('')
  const [propDescription, setPropDescription] = useState('')
  const [propPoolId, setPropPoolId] = useState('')
  const [propThreshold, setPropThreshold] = useState('5')
  const [propStatus, setPropStatus] = useState<string | null>(null)
  const [propSaving, setPropSaving] = useState(false)

  const loadProposals = () => {
    // Send the session cookie so the server can flag proposals authored by the
    // current verified member (drives the author-only "Withdraw" control).
    fetch('/api/governance/proposals', { credentials: 'include' })
      .then(r => r.json())
  ...
  // Reload once the member verifies (or changes identity) so the `mine` flag —
  // and therefore the Withdraw control — reflects who is actually signed in.
  useEffect(() => { loadProposals() }, [verifiedEmail])

  useEffect(() => {
    fetch('/api/data-pools')
      .then(r => r.json())
      .then(d => setDataPools((d.pools ?? []).map((p: any) => ({ id: p.id, name: p.name }))))
      .catch(() => {})
  }, [])
  ...
  const submitProposal = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!verifiedEmail) {
      setPropStatus('Verify your member email above before submitting a proposal.')
      return
    }
    setPropSaving(true); setPropStatus(null)
    try {
      const r = await fetch('/api/governance/proposals', {
        ...
        body: JSON.stringify({
          ...
          poolId: propPoolId || null,
          threshold: Number(propThreshold),
        }),
      })
      const d = await r.json()
      if (d.success) {
        setPropStatus('Proposal submitted — it is now open for community voting.')
        setPropTitle(''); setPropDescription(''); setPropPoolId(''); setPropThreshold('5')
        setShowProposalForm(false)
        loadProposals()
      } else {
        setPropStatus(d.error ?? 'Could not submit proposal.')
      }
    } catch {
      setPropStatus('Network error. Please try again.')
    }
    setPropSaving(false)
  }

  const castVote = async (proposalId: number, choice: 'yes' | 'no') => {
    if (!verifiedEmail) {
      setVoteStatus(s => ({ ...s, [proposalId]: 'Verify your member email above before voting.' }))
      return
    }
    setVoteStatus(s => ({ ...s, [proposalId]: 'saving' }))
    try {
      const r = await fetch(`/api/governance/proposals/${proposalId}/vote`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      const d = await r.json()
      if (d.success) {
        setVoteStatus(s => ({ ...s, [proposalId]: d.status === 'approved' ? 'Vote recorded — threshold met, proposal approved.' : 'Vote recorded.' }))
        loadProposals()
        if (expandedVotes === proposalId) loadVoteHistory(proposalId)
      } else {
        setVoteStatus(s => ({ ...s, [proposalId]: d.error ?? 'Vote failed.' }))
      }
    } catch {
      setVoteStatus(s => ({ ...s, [proposalId]: 'Network error.' }))
    }
  }

  const withdrawProposal = async (proposalId: number) => {
    if (!verifiedEmail) {
      setVoteStatus(s => ({ ...s, [proposalId]: 'Verify your member email above to withdraw your proposal.' }))
      return
    }
    if (!window.confirm('Withdraw this proposal? It will be closed to further voting. This cannot be undone.')) {
      return
    }
    setVoteStatus(s => ({ ...s, [proposalId]: 'saving' }))
    try {
      const r = await fetch(`/api/governance/proposals/${proposalId}/close`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      })
      const d = await r.json()
      if (d.success) {
        setVoteStatus(s => ({ ...s, [proposalId]: 'Proposal withdrawn.' }))
        loadProposals()
      } else {
        setVoteStatus(s => ({ ...s, [proposalId]: d.error ?? 'Could not withdraw proposal.' }))
      }
    } catch {
      setVoteStatus(s => ({ ...s, [proposalId]: 'Network error.' }))
    }
  ...
  const loadVoteHistory = (proposalId: number) => {
    fetch(`/api/governance/proposals/${proposalId}/votes`)
      .then(r => r.json())
      .then(d => setVoteHistory(d.votes ?? []))
      .catch(() => setVoteHistory([]))
  }

  const toggleVoteHistory = (proposalId: number) => {
    if (expandedVotes === proposalId) {
      setExpandedVotes(null)
    } else {
      setExpandedVotes(proposalId)
      loadVoteHistory(proposalId)
    }
  }
  ...
  return (
    <div className="man-app">
        ...
        <div className="solvy-dashboard">
          ...
          <div className="metrics-grid">
            ...
            <div className="metric-card card-pool">
              <div className="metric-label">Membership Pool (70%)</div>
              <div className="metric-value">$2,427</div>
              <div className="metric-change positive">Distribution decided at maturity</div>
              <div className="metric-description">70% of all revenue, held for members — distribution decided later, at maturity, with DECIDEY financial education.</div>
              <div className="metric-icon">🏦</div>
            </div>
          </div>
          ...
          <table className="rate-table">
            ...
            <tbody>
              {[
                ...
                ['Profit distribution', 'Shareholders (0% to workers)', '70% membership pool — held for members', '70/20/10 allocation tracker'],
      ...
      <section className="interchange-section">
        <div className="interchange-container">
          ...
          <div className="allocation-bar">
            ...
            <div className="allocation-segment pool-segment" style={{ width: '20%' }}>20% Operations</div>
          ...
          <div className="allocation-grid">
            <div className="allocation-card operational">
              <h2>Membership Pool</h2>
              <div className="stat-label">70% of Revenue</div>
              <div className="stat-large">${patronageAmount.toFixed(2)}</div>
              <p>Held for members — distribution is decided later, at maturity, guided by DECIDEY financial education. Direct descendant protection — revenue pooled for those who generate it.</p>
            ...
            <div className="allocation-card member-pool">
              ...
              <p>Pays for services — BaaS, the self-sovereign software stack, key employees, and member-approved operational costs.</p>
      ...
      {/* Member Governance — Data-Use Votes */}
      <section className="interchange-section" id="governance">
        <div className="interchange-container">
          <h1 className="interchange-title">Member Governance — Data-Use Votes</h1>
          <p className="interchange-subtitle">No change to how pooled data is used takes effect without a recorded member vote. Every vote below is logged and counted toward the approval threshold.</p>

          <div style={{ marginBottom: '1.5rem' }}>
            <MemberVerify
              onChange={setVerifiedEmail}
              prompt="Verify your member email to cast a vote or raise a proposal. One vote per member per proposal — you can change your vote while voting is open."
            />
          </div>

          {/* Member-raised proposals */}
          <div style={{ marginBottom: '1.5rem' }}>
            {!showProposalForm ? (
              <button
                onClick={() => { setShowProposalForm(true); setPropStatus(null) }}
                style={{ padding: '0.6rem 1.4rem', background: 'linear-gradient(135deg, #7c3aed, #5b21b6)', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 800, cursor: 'pointer', fontSize: '0.9rem' }}
              >
                ＋ Raise a Proposal
              </button>
            ) : (
              <form onSubmit={submitProposal} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(147,51,234,0.25)', borderRadius: '16px', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <h3 style={{ color: '#1a365d', fontSize: '1.05rem', fontWeight: 800, margin: 0 }}>Raise a New Data-Use Proposal</h3>
                <p style={{ color: '#64748b', fontSize: '0.82rem', margin: 0 }}>Describe the data-use change for the community to vote on. Keep it free of personal data — no emails, phone numbers, or account/ID numbers.</p>
                ...
                <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.82rem', fontWeight: 700, color: '#334155' }}>
                  ...
                  <input
                    ...
                    maxLength={160}
                    placeholder="e.g. License Remittance Behavior data to a research partner"
                    style={{ padding: '0.6rem 0.8rem', borderRadius: '8px', border: '1px solid rgba(147,51,234,0.3)', fontSize: '0.9rem', fontWeight: 400 }}
                  />
                </label>
                ...
                <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.82rem', fontWeight: 700, color: '#334155' }}>
                  ...
                  <textarea
                    ...
                    maxLength={2000}
                    rows={4}
                    placeholder="What change is being proposed, and how does revenue or retention work? No individual records are ever shared."
                    style={{ padding: '0.6rem 0.8rem', borderRadius: '8px', border: '1px solid rgba(147,51,234,0.3)', fontSize: '0.9rem', fontWeight: 400, resize: 'vertical' }}
                  />
                </label>

                <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                  <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.82rem', fontWeight: 700, color: '#334155', flex: '1 1 220px' }}>
                    Related Data Pool (optional)
                    <select
                      value={propPoolId}
                      onChange={e => setPropPoolId(e.target.value)}
                      style={{ padding: '0.6rem 0.8rem', borderRadius: '8px', border: '1px solid rgba(147,51,234,0.3)', fontSize: '0.9rem', fontWeight: 400 }}
                    >
                  ...
                  <label style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.82rem', fontWeight: 700, color: '#334155', flex: '0 1 160px' }}>
                    Approval Threshold (yes votes)
                    <input
                      type="number"
                      min={1}
                      max={1000}
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  <button type="submit" disabled={propSaving} style={{ padding: '0.55rem 1.4rem', background: 'linear-gradient(135deg, #16a34a, #15803d)', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 800, cursor: 'pointer', fontSize: '0.9rem' }}>
                    {propSaving ? 'Submitting…' : 'Submit Proposal'}
                  </button>
                  <button type="button" onClick={() => { setShowProposalForm(false); setPropStatus(null) }} style={{ padding: '0.55rem 1.4rem', background: 'rgba(100,116,139,0.1)', color: '#475569', border: '1px solid rgba(100,116,139,0.3)', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem' }}>
                    Cancel
                  </button>
                </div>
              </form>
            ...
            {propStatus && (
              <div style={{ marginTop: '10px', fontSize: '0.85rem', color: propStatus.includes('submitted') ? '#16a34a' : '#b91c1c' }}>{propStatus}</div>
            )}
          </div>

          {proposals.length === 0 ? (
            <p style={{ color: '#64748b', textAlign: 'center', padding: '1.5rem' }}>No active proposals.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              {proposals.map(p => {
                const approved = p.status === 'approved'
                const withdrawn = p.status === 'withdrawn'
                const open = !approved && !withdrawn
                const progress = Math.min(100, Math.round((p.yes_votes / p.threshold) * 100))
                const borderColor = approved ? 'rgba(34,197,94,0.4)' : withdrawn ? 'rgba(100,116,139,0.3)' : 'rgba(147,51,234,0.25)'
                const badgeStyle = approved
                  ? { background: '#dcfce7', color: '#166534' }
                  : withdrawn
                    ? { background: '#e2e8f0', color: '#475569' }
                    : { background: '#ede9fe', color: '#5b21b6' }
                const badgeLabel = approved ? '✓ Approved' : withdrawn ? 'Withdrawn' : 'Open for Voting'
                return (
                  <div key={p.id} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${borderColor}`, borderRadius: '16px', padding: '1.5rem', opacity: withdrawn ? 0.7 : 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', flexWrap: 'wrap' }}>
                      <h3 style={{ color: '#1a365d', fontSize: '1.1rem', fontWeight: 800, margin: 0, textDecoration: withdrawn ? 'line-through' : 'none' }}>{p.title}</h3>
                      <span style={{ ...badgeStyle, padding: '3px 12px', borderRadius: '20px', fontSize: '0.72rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', flexShrink: 0 }}>{badgeLabel}</span>
                    </div>
                    <p style={{ color: '#64748b', fontSize: '0.78rem', fontWeight: 600, margin: '4px 0 0' }}>Proposed by {p.author}</p>
                    <p style={{ color: '#4a5568', fontSize: '0.9rem', lineHeight: 1.6, margin: '10px 0 16px' }}>{p.description}</p>

                    <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '12px' }}>
                      <div style={{ fontSize: '0.85rem', color: '#166534', fontWeight: 700 }}>👍 {p.yes_votes} Yes</div>
                      <div style={{ fontSize: '0.85rem', color: '#b91c1c', fontWeight: 700 }}>👎 {p.no_votes} No</div>
                      <div style={{ fontSize: '0.82rem', color: '#64748b' }}>Threshold: {p.threshold} yes votes</div>
                    </div>

                    <div style={{ background: '#e2e8f0', borderRadius: '20px', height: '10px', overflow: 'hidden', marginBottom: '14px' }}>
                      <div style={{ width: `${progress}%`, height: '100%', background: approved ? '#16a34a' : 'linear-gradient(90deg, #9333ea, #7c3aed)', transition: 'width 0.3s' }} />
                    </div>

                    {open && (
                      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                        <button onClick={() => castVote(p.id, 'yes')} disabled={voteStatus[p.id] === 'saving'} style={{ padding: '0.55rem 1.4rem', background: 'linear-gradient(135deg, #16a34a, #15803d)', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem' }}>Vote Yes</button>
                        <button onClick={() => castVote(p.id, 'no')} disabled={voteStatus[p.id] === 'saving'} style={{ padding: '0.55rem 1.4rem', background: 'rgba(185,28,28,0.1)', color: '#b91c1c', border: '1px solid rgba(185,28,28,0.3)', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem' }}>Vote No</button>
                        {p.mine && (
                          <button onClick={() => withdrawProposal(p.id)} disabled={voteStatus[p.id] === 'saving'} style={{ marginLeft: 'auto', padding: '0.55rem 1.4rem', background: 'rgba(100,116,139,0.1)', color: '#475569', border: '1px solid rgba(100,116,139,0.35)', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem' }}>Withdraw</button>
                        )}
                      </div>
                    )}

                    {voteStatus[p.id] && voteStatus[p.id] !== 'saving' && (
                      <div style={{ marginTop: '10px', fontSize: '0.85rem', color: (voteStatus[p.id].includes('recorded') || voteStatus[p.id].includes('withdrawn')) ? '#16a34a' : '#b91c1c' }}>{voteStatus[p.id]}</div>
                    )}

                    <button onClick={() => toggleVoteHistory(p.id)} style={{ marginTop: '12px', background: 'none', border: 'none', color: '#7c3aed', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 700, padding: 0 }}>
                      {expandedVotes === p.id ? '▲ Hide vote history' : '▼ Show vote history'}
                    </button>

                    {expandedVotes === p.id && (
                      <div style={{ marginTop: '12px', borderTop: '1px solid rgba(147,51,234,0.15)', paddingTop: '12px' }}>
                        {voteHistory.length === 0 ? (
                          <p style={{ color: '#64748b', fontSize: '0.85rem', margin: 0 }}>No votes cast yet.</p>
                        ) : (
                          <table className="rate-table" style={{ marginTop: 0 }}>
                            <thead>
                              <tr><th>Member</th><th>Vote</th><th>When</th></tr>
                            </thead>
                            <tbody>
                              {voteHistory.map((v, i) => (
                                <tr key={i}>
                                  <td>{v.voter}</td>
                                  <td style={{ color: v.choice === 'yes' ? '#166534' : '#b91c1c', fontWeight: 700 }}>{v.choice === 'yes' ? 'Yes' : 'No'}</td>
                                  <td style={{ color: '#64748b' }}>{new Date(v.created_at).toLocaleString()}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
          ...
          <div className="highlight-box" style={{ marginTop: '1.5rem' }}>
            <h3>🗳️ How Governance Works</h3>
            <p>Only members can vote — your email must be on the member roster (founding member, prelaunch commitment, First Circle deposit, or a data-pool opt-in). Each member gets one vote per proposal. When a proposal reaches its yes-vote threshold, it is automatically marked approved and the data-use change takes effect. Vote history is permanently viewable here. Manage your own local data at <a href="/my-data" style={{ color: '#5b21b6' }}>My Data</a>.</p>
          </div>
        </div>
      </section>
