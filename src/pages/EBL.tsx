...
type ServicePrice = { default: string; label: string }
...
function EBL() {
  ...
  useEffect(() => {
    fetch('/api/ebl/prices')
      .then((r) => r.json())
      .then((data) => setServicePrices(data.prices ?? {}))
  ...
  useEffect(() => {
    ...
    if (status === 'success') {
      setPayNotice('Payment received — thank you! Eva has been notified and will be in touch.')
    } else if (status === 'cancelled') {
      setPayError('Payment was cancelled. You can try again whenever you are ready.')
    }
    if (status) {
  ...
  async function handlePay() {
    ...
    setPayNotice('')
    if (!serviceType) {
      setPayError('Please select a service first.')
      return
    }
    ...
    setPaying(true)
    try {
      const res = await fetch('/api/ebl/checkout', {
        method: 'POST',
  ...
  function handleServiceChange(val: string) {
    setServiceType(val)
    if (val && servicePrices[val] && !amountEdited) {
      setAmount(servicePrices[val].default)
    }
    if (!val) {
      if (!amountEdited) setAmount('')
    }
  }
  ...
  return (
    <div className="ebl-app">
      ...
      {/* Embedded ebl.beauty Site */}
      <section className="ebl-embed-section" id="visit-site">
        <div className="container">
          <h2>Visit Evergreen Beauty Lounge</h2>
          <p className="ebl-embed-subtitle">
            Browse services and book directly on ebl.beauty — right here without leaving SOLVY.
          </p>
          <div className="ebl-embed-frame-wrapper">
            <iframe
              src="https://ebl.beauty"
              title="Evergreen Beauty Lounge — ebl.beauty"
              className="ebl-embed-frame"
              loading="lazy"
          ...
          <a
            ...
            className="ebl-embed-open-link"
          >
            Open ebl.beauty in a new tab ↗
          </a>
        </div>
      </section>
        ...
        <div className="container">
          ...
          <div className="payment-form">
            ...
            <div className="form-group">
              ...
              <select value={serviceType} onChange={(e) => handleServiceChange(e.target.value)}>
            ...
            <div className="form-group">
              ...
              />
              {serviceType && servicePrices[serviceType] && servicePrices[serviceType].label && (
                <p className="price-hint">{servicePrices[serviceType].label} — adjust if agreed otherwise</p>
              )}
            </div>

            {payError && <p className="pay-message pay-error">{payError}</p>}
            {payNotice && <p className="pay-message pay-success">{payNotice}</p>}

            <div className="payment-buttons">
              <button className="btn-other-pay" onClick={handlePay} disabled={paying}>
                {paying ? 'Starting checkout…' : 'Pay with Card'}
              </button>
              <button className="btn-solvy-pay" disabled title="Available once your SOLVY debit card ships">
                Tap SOLVY Card — Coming Soon
              </button>
            </div>

            <p className="pay-secure-note">🔒 Secure checkout powered by Stripe. SOLVY debit card payments launch soon.</p>
