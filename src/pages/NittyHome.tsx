...
function NittyHome() {
  ...
  const [hintVisible, setHintVisible] = useState(() => {
    try {
      return localStorage.getItem('solvy_card_hint_seen') !== 'true'
    } catch {
      return true
    }
  })
  ...
  const dismissHint = () => {
    ...
    try {
      localStorage.setItem('solvy_card_hint_seen', 'true')
    } catch {
      // localStorage unavailable; hint will simply re-appear on next load
    }
  }
    ...
    <div className="app">
      ...
      <section className="hero" id="home">
        <div className="hero-content">
          ...
          <div className="hero-card-scene-wrapper">
            <div
              className={`hero-card-scene ${cardFlipped ? 'flipped' : ''}`}
              onMouseEnter={() => {
                if (!isTouchDevice) {
                  setCardFlipped(true)
                  dismissHint()
              ...
              <div className="hero-card-inner">
                ...
                <div className="hero-card-back">
                  <div className="card-back-content">
                    <div className="card-back-logo">SOLVY Ecosystem™</div>
                    <div className="card-back-stats">
                      <div className="card-stat">
                        <span className="card-stat-value">70%</span>
                        <span className="card-stat-label">Revenue back to members</span>
                      </div>
                      <div className="card-stat">
                        <span className="card-stat-value">0–1%</span>
                        <span className="card-stat-label">Interchange earned per swipe</span>
                      </div>
                      <div className="card-stat">
                        <span className="card-stat-value">P2P</span>
                        <span className="card-stat-label">Instant member-to-member transfers</span>
                      </div>
                      <div className="card-stat">
                        <span className="card-stat-value">🔒</span>
                        <span className="card-stat-label">Military-grade privacy mode</span>
                      </div>
                    </div>
                    <div className="card-back-cta">Cooperative Owned · Data Sovereign</div>
                  </div>
                  {isTouchDevice && cardFlipped && (
                    <div className="card-back-close">
                      <span className="card-back-close-icon">↺</span>
                      Tap to close
                    </div>
                  )}
                </div>
            ...
            <div className={`flip-hint ${hintVisible ? '' : 'hint-hidden'}`}>
              <span className="flip-hint-icon">↺</span>
              {isTouchDevice ? 'Tap to reveal stats' : 'Hover to reveal stats'}
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: 'linear-gradient(135deg, #0f1e2c 0%, #1e3a5f 100%)', padding: '48px 24px' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', flexWrap: 'wrap', marginBottom: '40px' }}>
            {[
              { name: 'Card Issuing Partner', role: 'Debit Card Infrastructure', icon: '💳', highlight: false },
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '36px', maxWidth: '900px', margin: '0 auto 36px' }}>
            {[
              ...
              { icon: '💳', title: 'Card Issuing Infrastructure', body: 'A regulated card-issuing partner powers the cooperative SOLVY Card — finalized at launch.' },
              { icon: '🔁', title: 'Crypto/Fiat Flexibility', body: 'On/off-ramp between crypto and fiat is on the roadmap — vendor-agnostic by design.' },
